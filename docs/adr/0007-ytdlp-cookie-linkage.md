...
4. **正本**: **Cookie linkage effective state** は yt-dlp user config が正。**Cookie linkage draft**（方式・ブラウザ・パスの下書きと **Cookie linkage risk acknowledgment**）はアプリ側。有効時の変更は **即時書き込み**（原則）。**Browser cookie source** ⇔ **Cookies file source** のラジオ切替に限る例外（[Issue #219](https://github.com/JO3QMA/vrctweaker/issues/219)）: Effective が有効のとき、切替先が書き込み可能なら即 upsert（Cookies file → Browser、Browser → Cookies file で有効パスありなど）。切替先がまだ書き込めない（典型: Cookies file source へ切替だがパス未指定／不存在）ときは **Disable** を先に呼び、案内文は出さず Draft を切替先方式のまま、トグル OFF とする。トグル OFF 中の参照は Draft 更新のみ（自動再有効化しない）。Disable 失敗時はセクション内エラー、ラジオは操作前 Effective へ戻し、トグル ON のまま。同一方式内（ブラウザ変更・パス欄編集）はこの例外に含めない。書き込み失敗時（上記以外）はエラー表示、UI を操作前の Effective に戻し、Draft の試行値は残す。config **未作成**の読み取りはエラーにせず Effective＝無効。**Cookie linkage config read failure**（存在するが読めない）は UI エラーとし、Effective を無効と偽らず、書き込みは止める。**risk acknowledgment** は Tweaker が初めて config へ書き込む操作の前に必須（他ツール由来で Effective が既に有効でも、閲覧のみなら不要）。ack 後の書き込みでは再確認しない
5. **UI**: 動画タブの独立セクション（Tools replace maintain の下。Config＝VRChat `config.json` 画面や Settings には載せない）。初回書き込み前に risk acknowledgment。以降は常時警告文。Cookie linkage の操作エラーは小見出し直後・常時警告の直上（置換ブロックの alert 群と同型）。v1 は **Windows のみ**表示（他 OS はセクション非表示）。**Tools replace effective state** が偽のときは **Cookie linkage official hint**（同一画面の Tools replace 操作への案内）を出すが、有効化・書き込みは止めない（ソフト結合）。ブラウザロック時の失敗は自動検知せず、ツールチップ等で「ブラウザを閉じる／Cookies file source を使う」を案内するにとどめる
