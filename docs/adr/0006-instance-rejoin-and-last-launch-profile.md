...
| **App (Wails)** | `nil, nil` | **`nil, error`**（frontend で通知） |
| **Frontend** | DTO `null` → section 非表示 | `ElMessage.error`、section 非表示 |

DTO には **Play session ID**（TOCTOU 防止用、instance key は含めない）、Rejoin target 表示名、Launch profile 一覧、初期選択 profile ID を含める。

### 起動 — `InstanceRejoin(profileID, playSessionID string) error`

- `profileID` / `playSessionID` 空 → **`error`**
- `playSessionID` が最新 Rejoin target でない → **`error`**（`rejoin target changed`）
- profile 未存在 → **`error`**
...
| 境界 | 方針 |
|------|------|
| **Frontend** | DTO に **VRChat instance key を含めない**（`playSessionId` と表示名のみ）。Join は `InstanceRejoin(profileID, playSessionID)` がサーバ側で Rejoin target を検証・解決 |
...
| 入力 | 期待 | テスト名 |
|------|------|----------|
| usecase infra `error` | App `error` | `TestGetInstanceRejoinSection_returnsErrorOnInfraFailure` |
| stale `playSessionID` | `error` | `TestInstanceRejoin_stalePlaySessionID` |
| `ResolveRejoinTarget` 最新一致 | target 返却 | `TestResolveRejoinTarget_stillLatest` |
...
| Wails | `GetInstanceRejoinSection`、`InstanceRejoin(profileID, playSessionID)` |
