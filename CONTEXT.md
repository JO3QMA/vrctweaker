...
**Output log ingest**:
VRChat の output_log.txt を読み取り、User encounter・Play session・ワールド表示名など Activity の元データを更新すること。起動時の過去分取り込みと、稼働中の追記監視を含む。監視対象がログフォルダ（ディレクトリ）のときは複数 Log source の並行 ingest を行う。単一ファイルを直接指定したときは Log source は 1 つのみ。
...
**Log replay**:
Output log ingest のうち、すでにディスク上にある行を offset から読み直して Activity の相関状態を再構築すること。起動時 bootstrap、単一ファイル監視のローテーション／truncate 後の再読み込みを含む。User encounter・Play session の更新のみ行い、Friend joined などの automation は発火しない（automation は追記監視の live tail に限る）。
