...
Automation の event / action がどの OS で有効かの方針。v1 では評価基盤（rule・script・スケジュール・ログ event・`change_status`）は**クロスプラットフォーム**、`set_power_plan` は **Windows のみ**。カタログの platform 宣言に従い、GUI は非対応 action を出さないか無効化し、Lua は実行時に明示エラーとする。
