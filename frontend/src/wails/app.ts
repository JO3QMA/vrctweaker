...
interface AppBindings {
  ...
  Login(username: string, password: string, twoFactorCode?: string): Promise<LoginResultDTO>
  Logout(): Promise<void>
...
export async function callApp<T>(fn: (app: AppBindings) => Promise<T>, fallback: T): Promise<T> {
  const app = getApp()
  if (!app) {
    return fallback
  }
...
export const App = {
  ...
  async searchScreenshots(filter: ScreenshotSearchDTO): Promise<ScreenshotDTO[]> {
  ...
  async login(username: string, password: string, twoFactorCode?: string): Promise<LoginResultDTO> {
    return callApp(
      (a) => a.Login(username, password, twoFactorCode ?? ''),
      { ok: false, error: 'App not available' },
    )
  },
  ...
  async getActivityStats(fromISO: string, toISO: string): Promise<ActivityStatsDTO> {
