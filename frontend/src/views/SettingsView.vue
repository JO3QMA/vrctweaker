      ...
      <div class="path-settings">
        <div class="path-row">
          ...
          <span
            ...
            :class="validateResult.vrchatPathWindows ? 'validate-ok' : 'validate-ng'"
          >
        ...
        <div class="path-row">
          ...
          <span
            ...
            :class="validateResult.steamPathLinux ? 'validate-ok' : 'validate-ng'"
          >
        ...
        <div class="path-row">
          ...
          <span
            ...
            :class="validateResult.outputLogPath ? 'validate-ok' : 'validate-ng'"
          >
...
async function logout() {
  ...
  try {
    ...
    loginError.value = e instanceof Error ? e.message : 'ログアウトに失敗しました'
  }
}
...
async function refreshFriends() {
  ...
  try {
    ...
    loginError.value = e instanceof Error ? e.message : 'フレンド一覧の更新に失敗しました'
  }
}
...
function runWithConfirm(
  ...
  fn()
    .then((result) => {
      const msg = successMessage ? successMessage(typeof result === 'number' ? result : undefined) : '完了しました'
      if (msg) {
        maintenanceError.value = ''
        window.alert(msg)
      }
    ...
    .catch((e) => {
      maintenanceError.value = e instanceof Error ? e.message : '操作に失敗しました'
    })
    .finally(() => {
...
.settings-section h2 { font-size: 1.1rem; margin: 0 0 1rem; }
.setting-row { display: flex; align-items: center; gap: 1rem; margin-bottom: 0.5rem; }
.setting-row input { width: 80px; padding: 0.4rem; background: var(--bg-tertiary); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-primary); }

.path-settings { display: flex; flex-direction: column; gap: 1rem; }
.path-row { display: flex; flex-direction: column; gap: 0.25rem; }
.path-row label { font-size: 0.95rem; }
.path-input-group { display: flex; gap: 0.5rem; align-items: center; }
...
.btn-validate:hover:not(:disabled) { opacity: 0.9; }
.btn-validate:disabled { opacity: 0.5; cursor: not-allowed; }
.validate-ok { font-size: 0.85rem; color: var(--success, #22c55e); }
.validate-ng { font-size: 0.85rem; color: var(--error, #ef4444); }

.hint { font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.5rem; }

/* Login section */
.login-status { display: flex; flex-direction: column; gap: 0.75rem; }
.logged-in-label { font-size: 0.95rem; color: var(--success, #22c55e); }
.login-actions { display: flex; gap: 0.5rem; flex-wrap: wrap; }
...
.btn-logout { background: var(--bg-tertiary); color: var(--text-primary); border: 1px solid var(--border); }
.btn-login:hover:not(:disabled), .btn-refresh:hover, .btn-logout:hover { opacity: 0.9; }
.btn-login:disabled { opacity: 0.5; cursor: not-allowed; }

.login-form { display: flex; flex-direction: column; gap: 1rem; max-width: 360px; }
.login-form .form-row { display: flex; flex-direction: column; gap: 0.25rem; }
.login-form .form-row label { font-size: 0.95rem; }
...
.login-error { font-size: 0.9rem; color: var(--error, #ef4444); margin: 0; }

/* DB Maintenance */
.maintenance-error { font-size: 0.9rem; color: var(--error, #ef4444); margin: 0 0 0.5rem; }
.maintenance-actions { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.btn-maintenance {
  padding: 0.5rem 1rem;
  border-radius: var(--radius);
  cursor: pointer;
...
.btn-maintenance:hover:not(:disabled) { opacity: 0.9; }
.btn-maintenance:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-maintenance-danger { border-color: var(--error, #ef4444); color: var(--error, #ef4444); }
</style>
