...
func TestProcessOutputLogFile_DispatchesEvents(t *testing.T) {
	...
	h := testEventHandler(func(ev activity.ParsedEvent) {
		mu.Lock()
...
func TestProcessOutputLogFile_topLevelWrapper(t *testing.T) {
	...
	err := ProcessOutputLogFile(context.Background(), path, activity.NewLogParser(), testEventHandler(func(activity.ParsedEvent) {
		count.Add(1)
	}), nil)
	if err != nil {
		t.Fatal(err)
...
func TestProcessOutputLogFileFromOffset_ResumeAndProgress(t *testing.T) {
	...
	parser := activity.NewLogParser()
	h := testEventHandler(func(activity.ParsedEvent) {})
	var progress []string
