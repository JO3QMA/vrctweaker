...
description: >-
  ...
  ブランチ作成、TDD での実装、fmt/test/lint（フロント時は E2E）検証、コードレビュー、PR 作成までの
...
- **実装の主戦場は Composer** とする（マルチファイル編集・ツール実行の中心）。
...
- 要件が曖昧なとき、設計が割れたとき、または QA が連続失敗するときは **[advisor-workflow](../advisor-workflow/SKILL.md)** に従い、**Chat または `mcp_task` で Claude Sonnet または Codex** に相談し、`.cursor/agents/advisor.md` の出力形式で短い指針を得てから Composer で実装を続ける。

**確認ポイント**: 要件が曖昧なときは解釈案を出して確認する（上記 advisor フロー併用可）。実装完了後、コミットメッセージに Issue 番号を含める（例: `fix: ... (#123)`）。

### Step 3: 検証

`.cursor/commands/run-verify.md` の手順に従い、`make fmt` → `make test` → `make lint` →（`frontend/src` を変更したら）`make test-e2e` を全パスするまで繰り返す。
...
| `run-verify.md` | fmt / test / lint（＋フロント時 E2E） |
...
- **`advisor-workflow`**: Composer メインで実装中に詰まったときの Sonnet / Codex 相談（回数上限あり）
